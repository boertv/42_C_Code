...
/*   Updated: 2023/06/30 16:42:18 by bvercaem         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	ps_sort_three(t_stack *a)
{
	...
	{
		if (!ps_rrotate(a, 1) || !ps_sort_three(a))
			return (0);
	}
	else if (a->start->nb > a->end->nb)
	{
		if (!ps_rotate(a, 1) || !ps_sort_three(a))
			return (0);
	}
	else if (!ps_swap(a, NULL, 1))
		return (0);
	return (1);
}
...
static int	ps_sort_five(t_stack *a, t_stack *b)
{
	...
	if (!ps_sort_three(a))
		return (0);
	if (!ps_push(b, a, 1) || !ps_push(b, a, 1))
		return (0);
	return (1);
}
